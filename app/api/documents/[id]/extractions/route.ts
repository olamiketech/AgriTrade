...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const extractions = await prisma.documentExtraction.findMany({
            where: { documentId: id },
            orderBy: { extractedAt: 'desc' } // or by name
        });
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const { extractionId, newValue, verified } = body;
        ...
        if (!extraction || extraction.documentId !== documentId) {
            return NextResponse.json({ error: 'Extraction not found or mismatch' }, { status: 404 });
        }
        ...
        const updated = await prisma.documentExtraction.update({
            where: { id: extractionId },
            data: {
                fieldValue: newValue, // Allow editing
        ...
        await createAuditLog(
            ...
            { extractionId, oldValue: extraction.fieldValue, newValue }
        );
